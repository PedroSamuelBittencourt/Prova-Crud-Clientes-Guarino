...
function mostrarPesquisado(){
    let containerPesquisado = document.getElementById("container-pesquisa")
    containerPesquisado.style.display = "flex"
}
...
function buscar(nome){
    ...
    for (let i = 0; i<clientes.length; i++) {
        let pesquisado = document.getElementById("pesquisado").value
        let cliente = clientes[i]
        if (cliente.nome == pesquisado){
            let linhapesquisado = `<td>${clientes[i].nome}</td>
            <td>${clientes[i].cpf}</td>
            <td>${clientes[i].telefone}</td>
            <td>${clientes[i].sexo}</td>
            <td>${clientes[i].time}</td>`

           document.getElementById("exibir").innerHTML = linhapesquisado
        }
    }
    mostrarPesquisado()
}
function fechar(){
     let containerModal = document.getElementById("container-pesquisa")
    containerModal.style.display = "none"
}